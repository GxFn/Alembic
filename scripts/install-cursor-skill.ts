 ...
 * - Agent 需要详情时调用 MCP: asd_knowledge({ operation: "get" }) / asd_search
 * - guard-context.md：同为轻量索引（fallback 用），Guard 主路径走 MCP asd_guard
 *
...
function buildProjectRecipesContext(projectRoot: any) {
  ...
  const lines = [
    ...
    'Agent 需要 Recipe 全文时请调用 MCP: `asd_knowledge({ operation: "get", id })` / `asd_search(query)`\n\n',
  ...
  lines.push('\n## Usage Tips\n\n');
  lines.push(
    '- 查找 Recipe: `asd_search({ query })` 或 `asd_search({ query, mode: "context" })`\n'
  );
  lines.push(
    '- 获取详情: `asd_knowledge({ operation: "get", id })` — 返回完整 Recipe 内容、关系、约束\n'
  );
  lines.push(
    '- 按类型浏览: `asd_knowledge({ operation: "list", kind: "rule" })` / `kind: "pattern"` / `kind: "fact"`\n'
  );
  lines.push('- Guard 检查: `asd_guard({ code })` / `asd_guard({ files })`\n');
...
// 可选：写入 MCP 配置，使 asd_search 等工具可用（连接层封装在此）
