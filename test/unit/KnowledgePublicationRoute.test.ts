...
function ctxWithServices(services: Record<string, unknown>): McpContext {
  return {
    container: {
      get(name: string) {
        if (name in services) {
...
function productionSession(overrides: Partial<Record<string, unknown>> = {}) {
  return {
    ...
    toJSON: vi.fn(() => ({ id: 'bs-asq', total: 1 })),
    ...overrides,
  };
}
...
describe('knowledge publication production route', () => {
      ...
      items: [
        {
          ...
          relations: {
            related: [
              { description: 'feeds search freshness validation', target: 'knowledge:k-asq' },
            ],
          },
  ...
  it('reports missing, invalid, and no-produce session diagnostics', () => {
    expect(
      describeSubmitKnowledgeProductionRoute(
          ...
          bootstrapSessionManager: {
            getSession: vi.fn(() =>
              productionSession({
                ...
                toJSON: vi.fn(() => ({ id: 'bs-asq', total: 0 })),
              })
            ),
          },
        }),
