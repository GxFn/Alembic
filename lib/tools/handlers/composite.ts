...
} from '#tools/core/ToolGuardServices.js';
import {
  getSearchEngine,
...
} from '#tools/core/ToolLifecycleServices.js';
import {
  getFeedbackCollector,
  resolveQualityServicesFromContext,
} from '#tools/core/ToolQualityServices.js';
import {
  checkDimensionType,
...
interface GatewayKnowledgeServiceLike {
  create(
...
function requireGatewayKnowledgeService(lifecycleServices: {
  getKnowledgeLifecycleService(): unknown | null;
}): GatewayKnowledgeServiceLike {
  const service = lifecycleServices.getKnowledgeLifecycleService();
  if (
    !service ||
    typeof service !== 'object' ||
    typeof (service as { create?: unknown }).create !== 'function'
  ) {
    throw new Error('Knowledge lifecycle service is not available in internal tool context');
  }
  const record = service as {
    create: GatewayKnowledgeServiceLike['create'];
    updateQuality?: GatewayKnowledgeServiceLike['updateQuality'];
  };
  return {
    create: record.create.bind(service),
    updateQuality:
      typeof record.updateQuality === 'function'
        ? record.updateQuality.bind(service)
        : async () => null,
  };
}
...
export const submitWithCheck = {
  ...
  handler: async (params: SubmitWithCheckParams, ctx: ToolHandlerContext) => {
    ...
    const item = {
      ...params,
      language: ctx._projectLanguage || params.language || '',
    ...
    try {
      ...
      const gateway = new RecipeProductionGateway({
        knowledgeService: requireGatewayKnowledgeService(lifecycleServices),
