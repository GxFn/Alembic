...
2. 使用 \`search_recipes\` / \`get_recipe_detail\` 获取旧知识上下文（必要时）
3. 使用 \`read_project_file\` 读取源文件，验证代码是否存在且与 Recipe 描述匹配
4. 如果源文件不存在或代码不匹配，使用 \`search_project_code\` / \`query_code_graph\` / \`query_call_graph\` 搜索该模式是否已迁移到其他位置
5. 基于真实代码的验证结果做出决策
...
- \`query_code_graph\` / \`query_call_graph\` — 验证符号、模块和调用关系，优先于文本猜测
...
export function buildEvolverPrompt(
  ...
  parts.push('- 使用 `query_code_graph` / `query_call_graph` 验证符号关系和调用链');
  parts.push('- 确认该模式是否迁移到了新位置或已被完全移除');
  parts.push('');
  if (strategyContext.terminalTest) {
    parts.push('**可选终端验证**');
    parts.push(
      `- 当前终端实验档位: ${strategyContext.terminalToolset || 'terminal-run'}；终端只用于验证工程命令事实`
    );
    parts.push('- 优先 terminal_run；只有管道/重定向必要时才用 terminal_shell');
