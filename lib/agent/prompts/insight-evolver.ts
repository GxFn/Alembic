...
| 验证结果 | 决策 | 调用方式 |
|---------|------|---------|
| 源文件存在 + 代码匹配 Recipe 描述 | **跳过**: 仍然有效 | \`knowledge({ action: "manage", params: { operation: "skip_evolution", recipeId: "...", reason: "..." } })\` |
| 源文件存在 + 代码已变化（接口改变/重构） | **进化提案**: 附加变更证据 | \`knowledge({ action: "manage", params: { operation: "evolve", recipeId: "...", ... } })\` |
| 源文件不存在 + 模式已迁移到新位置 | **进化提案**: 附加迁移证据 | \`knowledge({ action: "manage", params: { operation: "evolve", ... } })\` |
| 源文件不存在 + 完全无替代 | **确认废弃**: 知识已失效 | \`knowledge({ action: "manage", params: { operation: "deprecate", recipeId: "...", reason: "..." } })\` |
| 信息不足以做出判断 | **跳过**: 交给时限机制 | \`knowledge({ action: "manage", params: { operation: "skip_evolution", ... } })\` |

## 工具说明（统一使用 knowledge.manage）

- \`knowledge({ action: "manage", params: { operation: "evolve", recipeId, type, description, evidence, confidence } })\` — 为 Recipe 附加进化提案，提案进入观察窗口（enhance: 48h, correction: 24h）
- \`knowledge({ action: "manage", params: { operation: "deprecate", recipeId, reason } })\` — 确认 Recipe 已过时，立即标记为 deprecated
- \`knowledge({ action: "manage", params: { operation: "skip_evolution", recipeId, reason } })\` — 显式跳过，reason 中说明是"验证有效"还是"信息不足"
...
export function buildEvolverPrompt(
  ...
  parts.push('```json');
  parts.push(
    'knowledge({ action: "manage", params: { "operation": "deprecate", "recipeId": "recipe-xxx", "reason": "具体废弃原因" } })'
  );
  parts.push('```');
  parts.push('');
  ...
  parts.push('```json');
  parts.push(
    'knowledge({ action: "manage", params: { "operation": "skip_evolution", "recipeId": "recipe-xxx", "reason": "验证有效: 代码与描述完全匹配" } })'
  );
  parts.push('```');
  parts.push('或:');
  parts.push('```json');
  parts.push(
    'knowledge({ action: "manage", params: { "operation": "skip_evolution", "recipeId": "recipe-xxx", "reason": "信息不足: 无法确认源文件位置" } })'
  );
  parts.push('```');
