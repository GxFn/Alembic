...
export function hasNestedSandboxCommandConflict(command: string | undefined): boolean {
  const executable = firstExecutable(command);
  return executable ? hasNestedSandboxConflict(executable) : false;
}
...
function firstExecutable(command: string | undefined): string | null {
  if (!command) {
    return null;
  }
