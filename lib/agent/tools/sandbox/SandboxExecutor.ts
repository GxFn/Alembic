...
import {
  getSandboxExecPath,
  hasNestedSandboxCommandConflict,
...
export interface SandboxExecOptions {
  ...
  signal?: AbortSignal;
  stdin?: string;
  commandForConflictCheck?: string;
}
...
export async function sandboxExec(
  ...
  if (
    hasNestedSandboxConflict(options.bin) ||
    hasNestedSandboxCommandConflict(options.commandForConflictCheck)
  ) {
  ...
  if (profile.network.proxyPort === -1 && profile.network.allowedDomains.length > 0) {
    try {
      ...
      delete effectiveProfile.network.proxyPort;
    }
  }
  ...
  try {
    ...
    if (isSandboxApplyDenied(result)) {
      return directExec({ ...options, env: cleanEnv }, "sandbox-apply-denied");
    }
...
function isSandboxApplyDenied(result: { stderr: string; exitCode: number }): boolean {
  return result.exitCode === 71 && /sandbox_apply:\s+Operation not permitted/i.test(result.stderr);
}
