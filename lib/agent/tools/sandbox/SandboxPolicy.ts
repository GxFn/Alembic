...
export function buildSandboxProfile(input: SandboxInput): SandboxProfile {
  const globalMode = input.mode ?? getSandboxMode();
  if (globalMode === "disabled") {
    return {
      ...DISABLED_PROFILE,
      environment: {
        ...DISABLED_PROFILE.environment,
        inject: input.env ?? {},
      },
      limits: {
  ...
  const allowedDomains =
    input.network === "allowlisted" ? (input.allowedDomains ?? getConfiguredAllowedDomains()) : [];
  ...
  return {
    ...
    network: {
      allow: networkAllow,
      allowedDomains,
      ...(needsProxy ? { proxyPort: -1 } : {}), // -1 = SandboxExecutor 需动态分配
    },
    environment: {
