...
export async function fillDimensionsV3(fillContext) {
  ...
  async function executeDimension(dimId) {
    ...
    try {
      ...
      // ── 构建 per-dimension 的 stages (注入 onToolCall 闭包) ──
      const analyzeStage = {
        ...presetStages[0],
        onToolCall: (name, args, result, _iter) => {
          const ac = memoryCoordinator.getActiveContext(analystScopeId);
          if (ac) ac.recordToolCall(name, args, result, true);
        },
      };
      ...
      const runResult = await Promise.race([
        ...
        new Promise((_, reject) =>
          setTimeout(() => reject(new Error(`Bootstrap runtime timeout for "${dimId}"`)), 480_000),
        ),
      ]);
