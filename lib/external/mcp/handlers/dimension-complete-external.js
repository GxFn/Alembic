...
export async function dimensionComplete(ctx, args) {
  ...
  // referencedFiles 自动补全：若 Agent 未传递，从 SubmissionTracker 已记录的
  // reasoning.sources 中提取（去重 + 去掉行号后缀）
    ...
    try {
      ...
      if (knowledgeService) {
        for (const recipeId of submittedRecipeIds) {
          try {
            // 尝试通过 knowledgeService 给 recipe 打上 dimensionId 标签
            const entry = await knowledgeService.get(recipeId);
            if (entry) {
              // 附加 dimensionId 到 metadata（如果 API 支持 update）
  ...
  try {
    ...
    if (taskManager) {
      taskManager.markTaskCompleted(dimensionId, {
        type: dim.skillWorthy ? 'skill' : 'candidate',
        extracted: candidateCount || submittedRecipeIds.length,
        source: 'external-agent',
      });
    }
  } catch { /* taskManager not available */ }

  try {
    const eventBus = ctx.container.get('eventBus');
    if (eventBus) {
  ...
  if (
    ...
    evidenceHints = {
      ...
      usedTriggers: accumulatedEvidence.usedTriggers.length > 0
        ...
        : undefined,
      _note: '以上为前序维度的分析证据，请利用其中的文件引用和负空间信号，避免重复分析已覆盖的内容',
    };
  }
