  ...
  try {
    ...
    if (qualityScorer) {
      ...
      const scoreResult = qualityScorer.score({
        ...data,
        code: codeForScore,
        summary: data.description || data.content?.rationale || '',
      });
      // QualityScorer 维度 → Quality 值对象映射:
      //   completeness → completeness (内容完整度)
      //   metadata     → adaptation   (项目适配度: category/tags/summary 丰富度)
      //   format       → documentation(文档清晰度: trigger 格式 + language 合法性)
      const dims = scoreResult.dimensions || {};
      data.quality = {
        completeness: Math.round((dims.completeness ?? 0) * 100) / 100,
        adaptation:   Math.round((dims.metadata ?? 0) * 100) / 100,
        documentation: Math.round((dims.format ?? 0) * 100) / 100,
...
export async function submitKnowledgeBatch(ctx, args) {
  ...
  try {
    ...
    if (session?.submissionTracker) {
      ...
      currentDimId = progress.remainingDimIds[0] || args.target_name || 'unknown';
    }
  } catch { /* best effort */ }
      ...
      if (session?.submissionTracker && currentDimId) {
        try {
          session.submissionTracker.recordRejection(
            currentDimId, items[i].title || '(untitled)', readiness.missing.join(', '));
        } catch { /* best effort */ }
      }
      continue;
    }
