...
export async function bootstrapExternal(ctx) {
  ...
  const {
    ...
    activeDimensions: dimensions, skillContext, targetsSummary,
    langProfile,
  } = phaseResults;

  // skills 列表（用于 enrichDimensionTask）
  const skills = (skillContext?.loaded || []).map((name) => ({
    name,
    relatedDimension: name.replace('project-', ''),
    content: skillContext?.contents?.[name] || '',
  }));
