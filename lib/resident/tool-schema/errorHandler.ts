/**
 * Alembic resident tool 统一错误处理
 *
 * 提供 wrapHandler() 包装函数，将所有 resident handler 的异常统一转换为
 * envelope 格式的错误响应，确保：
 ...
 *
 * @module resident/tool-schema/errorHandler
 */
...
/** Handler function signature for resident tools */
type McpHandlerFn = (
...
/**
 * 包装 resident tool handler 函数，提供 Zod 输入校验 + 统一错误处理
 *
  ...
  return async function wrappedHandler(
    ...
    try {
      ...
      if (err instanceof z.ZodError) {
        ...
        logger.warn(`[ResidentTool:${toolName}] VALIDATION_ERROR: ${msg}`);
        return {
          content: [
            {
              type: 'text',
      ...
      logger.error(`[ResidentTool:${toolName}] ${errorCode}: ${message}`, {
        tool: toolName,
        errorCode,
        durationMs: elapsed,
