...
export class RetrievalFunnel {
  ...
  #contextAwareRerank(candidates, context) {
    ...
    return candidates
      .map((candidate) => {
        let boost = 0;
        const text = [candidate.title, candidate.trigger, candidate.content]
          .filter(Boolean)
          .join(' ')
          .toLowerCase();
        ...
        if (sessionOverlap > 0) {
          boost += 0.2 * Math.min(sessionOverlap / 5, 1);
        }

        // 语言匹配
        if (context.language && candidate.language === context.language) {
          boost += 0.1;
        }
