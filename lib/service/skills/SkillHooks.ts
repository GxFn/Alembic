 ...
 *
 * v2 升级:
 *   - 覆盖知识/Guard/Skill/搜索/Bootstrap 全生命周期
 ...
 *
 * 加载顺序: 包内置 skills/ → 项目级 Alembic/skills/（同名覆盖）
 */
...
export class SkillHooks {
  ...
  tap(
    ...
    if (!handlers) {
      throw new Error(`SkillHooks.tap: failed to register hook "${hookName}"`);
    }
  ...
  #addHandler(
    ...
    const handlers = this.hooks.get(hookName);
    if (!handlers) {
      throw new Error(`SkillHook not declared: ${hookName}`);
    }
