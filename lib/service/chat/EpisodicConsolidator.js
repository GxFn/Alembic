 ...
 *
 * Bootstrap 完成后，将 EpisodicMemory (Tier 2) 中的维度分析结果
...
export class EpisodicConsolidator {
  ...
  /**
   * 执行固化: EpisodicMemory → ProjectSemanticMemory
   *
   * @param {import('../../external/mcp/handlers/bootstrap/pipeline/EpisodicMemory.js').EpisodicMemory} episodicMemory
   * @param {object} [opts]
   ...
   */
  consolidate(episodicMemory, { bootstrapSession, clearPrevious = false } = {}) {
    const t0 = Date.now();
    ...
    const findingMemories = this.#extractFromFindings(episodicMemory);

    // 3. 从 Tier Reflections 提取 insight 记忆
    const insightMemories = this.#extractFromReflections(episodicMemory);

    // 4. 从 analysisText 提取文本中的事实
    const textFactMemories = this.#extractFromAnalysisText(episodicMemory);
   ...
   */
  #extractFromFindings(episodicMemory) {
    const memories = [];
    const completedDims = episodicMemory.getCompletedDimensions();
   ...
   */
  #extractFromReflections(episodicMemory) {
    const memories = [];
    const json = episodicMemory.toJSON();
   ...
   */
  #extractFromAnalysisText(episodicMemory) {
    const memories = [];
