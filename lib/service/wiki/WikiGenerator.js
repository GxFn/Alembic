...
export class WikiGenerator {
  ...
  _discoverTopics(projectInfo, astInfo, moduleInfo, knowledgeInfo) {
    ...
    } else if (sourceModuleKeys.length >= 2) {
      // 无 moduleService → 从 sourceFilesByModule 推断模块
    ...
    if (astSparse) {
      const folderProfiles = profileFolders(projectInfo, {
        minFiles: 3,
        maxFolders: 15,
      });
      ...
      if (folderProfiles.length > 0) {
        ...
        for (const fp of folderProfiles) {
          ...
          topics.push({
            id: `folder-${slug(fp.name)}`,
            path: `folders/${slug(fp.name)}.md`,
            title: fp.name,
            type: 'folder-profile',
        ...
        logger.info(
          `[WikiGenerator] Folder profiling (AST sparse): ${folderProfiles.length} folders analyzed, ` +
