  ...
  #prepareIteration(ctx) {
    ...
    if (tracker) {
      ...
      if (nudge) {
        ...
        this.logger.debug(`[AgentRuntime] Nudge [${nudge.type}] iter=${ctx.iteration}${_dim ? ` dim=${_dim}` : ''}: ${nudge.text.slice(0, 120)}`);
      }
    }
  ...
  async #processToolCalls(ctx, llmResult, effectiveSystemPrompt) {
    ...
    if (tracker) {
      ...
      if (transitionNudge) {
        ...
        this.logger.debug(`[AgentRuntime] Transition Nudge [${transitionNudge.type}] phase=${tracker.phase}${_dimT ? ` dim=${_dimT}` : ''}: ${transitionNudge.text.slice(0, 120)}`);
      }
    }
  ...
  #processTextResponse(ctx, llmResult) {
    ...
    if (tracker) {
      ...
      if (textResult.needsDigestNudge) {
        ...
        this.logger.debug(`[AgentRuntime] Digest Nudge [SUMMARIZE]${_dimD ? ` dim=${_dimD}` : ''}: ${textResult.nudge.slice(0, 120)}`);
        trace?.endRound?.();
      ...
      if (textResult.shouldContinue) {
        ...
        if (textResult.nudge) {
          ...
          this.logger.debug(`[AgentRuntime] Continue Nudge${_dimC ? ` dim=${_dimC}` : ''}: ${textResult.nudge.slice(0, 120)}`);
        }
        trace?.endRound?.();
        return false; // continue
      }
