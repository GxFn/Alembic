...
function buildConsolidationAction(input: {
  readonly decision: MainlineRecipeSubmissionDecision;
  ...
  return {
    ...
    similarRecipes,
    fieldMergeSuggestions: buildFieldMergeSuggestions(input.candidate, targetMatch?.recipe),
...
function buildFieldMergeSuggestions(
  ...
  const candidateRecord = candidateFields(candidate, candidate);
