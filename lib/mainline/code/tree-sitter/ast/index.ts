...
export async function loadPlugins() {
  ...
  await initParser();
  if (!isParserReady()) {
    throw new Error('web-tree-sitter initialized without a ready parser.');
  }
  ...
  for (let i = 0; i < LANG_REGISTRY.length; i++) {
    ...
    if (wasmResult.status !== 'fulfilled' || !wasmResult.value) {
      throw new Error(`Tree-sitter grammar failed to load: ${entry.wasmFile}`);
    }
    ...
    try {
      ...
      throw new Error(
        `Tree-sitter language plugin failed to load: ${entry.langId} (${error instanceof Error ? error.message : String(error)})`,
      );
    }
  }
}
