...
/** 预编译 .wasm 文件存放目录。新仓库保留 resources/grammars 作为插件包资源。 */
const GRAMMARS_DIR = await resolveGrammarsDir();
...
export async function initParser() {
  ...
  // 成熟 AST 链路是冷启动前工程事实层的硬依赖；缺依赖时应直接失败。
...
async function resolveGrammarsDir(): Promise<string> {
