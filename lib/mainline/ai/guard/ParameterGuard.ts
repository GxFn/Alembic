 ...
 *
 * 参数守卫只做“收窄”：删除模型禁止的参数、把越界值夹到允许范围，
 * 并把每个过滤动作记录到 filtered，方便调用方审计 provider 实际收到什么。
 */
...
// biome-ignore lint/complexity/noStaticOnlyClass: 参数守卫是无状态策略，静态入口能让 provider adapter 直接复用。
