...
export class TreeSitterMainlineAstParser implements MainlineAstParser {
  ...
  async parse(request: MainlineAstParseRequest): Promise<MainlineAstParseResult> {
    ...
    await initializeTreeSitterRuntime([parserLanguageId as EngineeringTreeSitterLanguageId]);
    if (!isAvailable(parserLanguageId)) {
      throw new Error(`Tree-sitter AST analyzer is not available for ${parserLanguageId}.`);
    }
