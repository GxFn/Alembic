...
import { type AstSummary, type DetectedPattern, EnhancementPack } from "../pack.js";
...
class VueEnhancement extends EnhancementPack {
  ...
  override preprocessFile(content: string, ext: string) {
    if (ext !== ".vue") {
      return null;
    }
    // 优先提取 <script setup>
    ...
    if (setupMatch) {
      const content = setupMatch[2];
      if (content !== undefined) {
        return { content, lang: setupMatch[1] ? "typescript" : "javascript" };
      }
    }
    ...
    if (scriptMatch) {
      ...
      if (content !== undefined) {
        return { content, lang: scriptMatch[1] ? "typescript" : "javascript" };
      }
    }
    return null;
  }

  override detectPatterns(astSummary: AstSummary): DetectedPattern[] {
