...
import { normalizeEngineeringDependencyNode } from "../foundation/EngineeringCoreTypes.js";
...
import {
  ...
  isLikelyThirdPartyEngineeringPath,
  toEngineeringRelativePath,
} from "../workspace/EngineeringWorkspacePaths.js";
import type {
  EngineeringPanoramaModuleSummary,
  EngineeringPanoramaSummary,
...
const SOURCE_WRAPPER_DIRS = new Set(["source", "sources", "src", "lib"]);
const TEST_WRAPPER_DIRS = new Set(["test", "tests", "__tests__", "spec", "specs"]);
...
export class EngineeringModuleDiscoverer {
  discover(input: EngineeringModuleDiscovererInput): EngineeringModuleDiscoveryResult {
    const files = normalizeFiles(input.projectRoot, input.files).filter((file) =>
      shouldUseFile(file.relativePath),
    );
...
function normalizeFiles(projectRoot: string, files: readonly EngineeringFile[]): NormalizedFile[] {
...
function shouldUseFile(relativePath: string): boolean {
...
function buildModuleDrafts(
  ...
  graphFacts: ReturnType<typeof readDependencyGraphFacts>,
  ...
  const hostFiles = new Map<string, NormalizedFile[]>();
...
function hostNameForFile(file: NormalizedFile, hostNames: ReadonlySet<string>): string | undefined {
...
function hostSubmoduleName(segments: readonly string[]): string | undefined {
  ...
  if (RESOURCE_DIRS.has(firstLower) || EngineeringLanguageProfiles.vendorDirs.has(first)) {
    return undefined;
  }
  return first;
}
...
function inferImportFallbackEdges(
...
function isHostNode(node: EngineeringDependencyNode): boolean {
...
function isExternalNode(node: EngineeringDependencyNode): boolean {
  return node.type === "external" || node.type === "remote" || node.indirect === true;
}

function isExternalDependencyName(name: string): boolean {
  return (
    name.startsWith("@") ||
    /^[a-z0-9_.-]+$/i.test(name) ||
    name.includes(".") ||
...
function normalizeImportPackage(specifier: string): string {
