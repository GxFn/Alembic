...
import type { DimensionCheckpointResult, IncrementalPlan } from '#external/mcp/handlers/types.js';
import Logger from '#infra/logging/Logger.js';
import type { BootstrapEventEmitter } from '#service/bootstrap/BootstrapEventEmitter.js';
import { loadCheckpoints } from '#workflows/deprecated-cold-start/checkpoint/BootstrapCheckpointStore.js';
import type {
  CandidateResults,
...
export interface BootstrapCheckpoint extends DimensionCheckpointResult {
  dimId?: string;
...
export async function restoreCheckpointDimensions({
  ...
  const completedCheckpoints = (await loadCheckpoints(dataRoot)) as Map<
    string,
    BootstrapCheckpoint
  >;
...
export function applyRestoredDimensionState({
  ...
  completedCheckpoints: Map<string, BootstrapCheckpoint>;
  sessionStore: SessionStore;
