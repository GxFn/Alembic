...
import {
  EXAMPLE_TEMPLATES,
  SUBMISSION_SCHEMA,
} from '#workflows/deprecated-cold-start/briefing/BootstrapDimensionText.js';
...
function buildExecutionPlan(activeDimensions: DimensionDef[]) {
  ...
  return {
    ...
    workflow:
      '对每个维度: (1) 用你的原生能力阅读代码分析 → (2) 调用 alembic_submit_knowledge_batch 批量提交候选（**每维度最少 3 条，目标 5 条**，将不同关注点拆分为独立候选，1-2 条视为不合格） → (3) 调用 alembic_dimension_complete 完成维度（必须传 referencedFiles=[分析过的文件路径] 和 keyFindings=[3-5条关键发现]）',
  };
}
