/**
 * IncrementalRescanWorkflow — Alembic API AI 增量知识重扫
 *
 * 结构清洗 W3——增量扫描归位 generate 环（自 sustain/KnowledgeRescanWorkflow 迁入；
 * 它是 GenerateWorkflow mode='incremental' 的实现）。
 *
...
} from '../../sustain/ProduceSessionPlan.js';
import {
...
import { writeKnowledgeRescanCoverageLedgerForDimension } from './RescanCoverageLedgerWriter.js';
import {
  buildKnowledgeRescanMiningPlanOptions,
  isRecord,
  knowledgeRescanMiningModeArg,
  nonNegativeInteger,
  positiveInteger,
  shouldRunInternalRescanFillInline,
} from './RescanMiningPlanArgs.js';
...
/**
 * runIncrementalRescanWorkflow — GenerateWorkflow mode='incremental' 的真实编排器
 * （W3 名实修正：更名自 runKnowledgeRescanProjectIndexWorkflow，日志前缀保持原文案不变）。
 */
export async function runIncrementalRescanWorkflow(
...
// 模块副作用注册：GenerateWorkflow 懒加载本模块（dynamic import）后，
// 经此调用取得 mode='incremental' 的实现。注册载体随 W3 从 sustain 壳迁到本文件。
registerGenerateWorkflowImplementation('incremental', runIncrementalRescanWorkflow);
