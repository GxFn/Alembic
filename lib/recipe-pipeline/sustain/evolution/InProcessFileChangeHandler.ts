/**
 * InProcessFileChangeHandler — daemon in-process file-change reactive evolution
 *
 * 核心策略：
...
import type { FileChangeSubscriber } from '../../../service/FileChangeDispatcher.js';
...
const HANDLER_NAME = 'InProcessFileChangeHandler';

export class InProcessFileChangeHandler implements FileChangeSubscriber {
  ...
  async handleFileChanges(events: FileChangeEvent[]): Promise<ReactiveEvolutionReport> {
    ...
    for (const event of events) {
      ...
      switch (event.type) {
        case 'renamed': {
          ...
          if (!newP) {
            this.#logger.warn(
              '[InProcessFileChangeHandler] renamed event missing target path, treating as deleted',
              { oldPath: oldP }
            );
    ...
    if (report.fixed > 0 || report.deprecated > 0 || report.needsReview > 0) {
      this.#logger.info('[InProcessFileChangeHandler] handleFileChanges complete', {
        fixed: report.fixed,
    ...
    for (const ref of affected) {
      try {
        ...
        if (gatewayResult.outcome === 'error') {
          this.#logger.warn('[InProcessFileChangeHandler] Gateway rename proposal failed', {
            recipeId: ref.recipeId,
        ...
        this.#logger.warn('[InProcessFileChangeHandler] rename proposal failed', {
          recipeId: ref.recipeId,
      ...
      try {
        ...
        if (activeRefs.length === 0) {
          ...
          if (gatewayResult.outcome !== 'error') {
            ...
            this.#logger.warn('[InProcessFileChangeHandler] Gateway deprecation failed', {
              recipeId: ref.recipeId,
              error: gatewayResult.error,
            });
            report.skipped++;
        ...
        this.#logger.warn('[InProcessFileChangeHandler] delete handling failed', {
          recipeId: ref.recipeId,
          error: (err as Error).message,
        });
        report.skipped++;
  ...
  #emitSourceModifiedSignal(
    ...
    try {
      this.#signalBus.send('quality', HANDLER_NAME, IMPACT_WEIGHTS[impactLevel], {
        target: recipeId,
        metadata: {
  ...
  #emitSignals(report: ReactiveEvolutionReport): void {
    ...
    try {
      if (report.fixed > 0) {
        this.#signalBus.send('quality', HANDLER_NAME, 0.1, {
          metadata: {
            reason: 'reactive_fix',
            fixed: report.fixed,
          },
