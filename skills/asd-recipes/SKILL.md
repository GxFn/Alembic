...
| **Snippets** | `Alembic/snippets/*.json` | Code snippets synced to IDE via `asd install` |
| **Candidates** | `Alembic/.asd/candidates.json` | AI-scanned candidates; review in Dashboard then approve |
| **Context index** | `Alembic/.asd/context/` | Vector index built by `asd embed`; semantic search via `asd_search(mode=context)` |
...
| Allowed | Forbidden |
|---------|-----------|
| Submit candidates (`asd_submit_knowledge` / `_batch`) | Directly create/modify Recipes |
| Search/query (`asd_search` / `asd_knowledge`) | Publish/deprecate/delete |
...
2. **MCP browse**: `asd_knowledge(operation=list)` with kind/language/category filters
3. **MCP get**: `asd_knowledge(operation=get, id)` for full content
4. **MCP search**: `asd_search(mode=auto)` for unified FieldWeighted+semantic search
...
4. **Guard/Audit**: `// as:audit` or MCP `asd_guard` — both use Recipes as standard
5. **Confirm adoption**: `asd_knowledge(operation=confirm_usage, id, usageType)` when user uses a Recipe
...
2. **From existing Recipes**: Check index for matching modules, then `asd_knowledge(operation=get, id)` for full content
3. **Via semantic search**: `asd_search(mode=context)` with query like "import ModuleName"
