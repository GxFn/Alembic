...
| 入口 | 文件 | 用途 |
|------|------|------|
| CLI | `bin/cli.js` | `alembic` 命令行工具，基于 commander，14 个子命令 |
...
| Ollama (本地) | `ALEMBIC_AI_PROVIDER=ollama` |
...
2. **SQLite as Cache** — Markdown 文件是 Source of Truth，SQLite 是读缓存，`alembic sync` 可重建
